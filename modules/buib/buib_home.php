...
// jika file di include oleh file lain, tampilkan isi file
    ...
    if (in_array($_SESSION['hak_akses'], ['SuperAdmin', 'buib', 'Pimpinan', 'SekretarisPimpinan'])) {
        ...
        $kategori_result = mysqli_query($mysqli, "SELECT id_deputy, nama_deputy FROM tbl_deputy_buib");
        $kategori_list = mysqli_fetch_all($kategori_result, MYSQLI_ASSOC);

        // Ambil data status
        $status_result = mysqli_query($mysqli, "SELECT id_status, nama_status FROM tbl_status");
                        ...
                        <div class="card-body">
                            <div class="row align-items-center">
                                <div class="col-md-3">
                                    ...
                                    <select class="form-control" id="filterTahun" onchange="updateChartsAndTables()">
                                ...
                                <div class="col-md-3">
                                    <label for="filterProgram" class="form-label">Filter Program:</label>
                                     <select class="form-control" id="filterProgram" onchange="updateChartsAndTables()">
                                        ...
                                        while ($program_data_option = mysqli_fetch_assoc($program_query)) {
                                            echo "<option value='" . htmlspecialchars($program_data_option['nama_program']) . "'>" . htmlspecialchars($program_data_option['nama_program']) . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <?php if (!empty($data['dokumen_rk_buib'])) : ?>
                                                <a href="<?php echo htmlspecialchars($data['dokumen_rk_buib']); ?>" target="_blank" rel="noopener noreferrer" class="btn btn-info btn-sm" title="Buka Dokumen">
        ...
        <script>
            ...
            // DITAMBAHKAN: fungsi refresh data
            function refreshData() {
                location.reload();
            }

            function filterData(tahun, program) {
                // ... (fungsi filterData tetap sama, tidak perlu diubah) ...
                ...
                filteredData.forEach(item => {
                    const tahunItem = new Date(item.tgl_surat).getFullYear().toString();
                    ...
                    if (!filteredYearlyData[tahunItem]) {
                        filteredYearlyData[tahunItem] = { target: 0, realisasi: 0 };
                    }
                    filteredYearlyData[tahunItem].target += parseFloat(item.target_nominal || 0);
                    filteredYearlyData[tahunItem].realisasi += parseFloat(item.realisasi_nominal || 0);
                    
                    if (!filteredMonthlyData[bulan]) {
                        filteredMonthlyData[bulan] = { label: bulanNama, target: 0, realisasi: 0, kontrak: 0, ongoing: 0 };
                    }
                ...
                Object.keys(filteredMonthlyData).sort().forEach(bulan => {
                    monthlyAccumulative[bulan] = {
                        ...
                        realisasi: filteredMonthlyData[bulan].realisasi,
                        terkontrak: filteredMonthlyData[bulan].kontrak,
                        ongoing: filteredMonthlyData[bulan].ongoing
                    };
                });
            ...
            // ... (Fungsi updateLineChart, updateDoughnutChart, updateBarChart, updateKategoriChart tetap sama) ...
            function updateLineChart(monthlyAccumulative) {
                ...
                lineChart = new Chart(document.getElementById('lineChart'), {
                    ...
                    data: {
                        labels: labels,
                        datasets: [
                            { label: 'Rencana Bulanan', data: targetData, borderColor: '#FF6384', tension: 0.4, fill: false },
                            { label: 'Realisasi', data: realisasiData, borderColor: '#36A2EB', tension: 0.4, fill: false },
                            { label: 'Kontrak', data: terkontrakData, borderColor: '#FFCE56', tension: 0.4, fill: false },
                            { label: 'Ongoing', data: ongoingData, borderColor: '#4BC0C0', tension: 0.4, fill: false }
                        ]
                    },
                    options: { responsive: true, maintainAspectRatio: false, /* ... rest of options ... */ }
                });
            }
            ...
            function updateBarChart(monthlyAccumulative) {
                ...
                barChart = new Chart(document.getElementById('barChart'), {
                    ...
                    data: {
                        labels: labels,
                        datasets: [
                            { label: 'Rencana Bulanan', data: targetData, backgroundColor: 'rgba(255, 99, 132, 0.7)' },
                            { label: 'Realisasi Bulanan', data: realisasiData, backgroundColor: 'rgba(54, 162, 235, 0.7)' }
                        ]
                    },
                    options: { responsive: true, maintainAspectRatio: false, /* ... rest of options ... */ }
                });
        ...
        function generateModal(id, type, isDelete = false) {
            ...
            const allKategori = <?php echo json_encode($kategori_list); ?>;
            const allStatus = <?php echo json_encode($status_list); ?>;
            ...
            if (isDelete) {
                // Modal Hapus (tidak ada perubahan di sini)
                return `
            ...
            // Bagian modal edit yang akan kita modifikasi
            // Bagian nominalFields tetap sama, tidak perlu diubah.
            let nominalFields = '';
            switch (type) {
                case 'realisasi':
                    nominalFields = `
                            ...
                            <form action="modules/buib/proses_ubah.php" method="POST" id="form${capitalizedType}${id}">
                                <div class="modal-body">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="deputy_buib" required>
                                                    ...
                                                    ${/* PERUBAHAN 1: Membuat dropdown Kategori dengan JavaScript */
                                                    allKategori.map(kategori => `
                                                        <option value="${kategori.id_deputy}" ${kategori.id_deputy == item.deputy_buib ? 'selected' : ''}>
                                                            ${kategori.nama_deputy}
                                                        </option>
                                                    `).join('')}
                                                </select>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="status_buib" required>
                                                    ...
                                                    ${/* PERUBAHAN 2: Membuat dropdown Status dengan JavaScript */
                                                    allStatus.map(status => `
                                                        <option value="${status.id_status}" ${status.id_status == item.status_buib ? 'selected' : ''}>
                                                            ${status.nama_status}
                                                        </option>
                                                    `).join('')}
                                                </select>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control" name="dokumen_rk_buib" value="${item.dokumen_rk_buib || ''}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="row">
                                    ...
                                    <div class="alert alert-light border mt-3">
                                        <h6 class="mb-3 text-center"> <i class="fas fa-history mr-2 text-info"></i>Data Sebelumnya
                                        </h6>
                                        <div class="row small">
