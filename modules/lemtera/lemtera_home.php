...
// jika file di include oleh file lain, tampilkan isi file
	...
	if (in_array($_SESSION['hak_akses'], ['SuperAdmin', 'LEMTERA', 'Pimpinan', 'SekretarisPimpinan'])) { 
        ...
        $kategori_result = mysqli_query($mysqli, "SELECT id_entity, nama_entity_lemtera FROM tbl_entity_lemtera");
        ...
        function filterData(tahun, program) {
            ...
            filteredData.forEach(item => {
                ...
                // Kategori data
                if (!filteredKategoriData[item.nama_entity_lemtera]) {
                    filteredKategoriData[item.nama_entity_lemtera] = { count: 0, realisasi: 0, target: 0 };
                }
                filteredKategoriData[item.nama_entity_lemtera].count++;
                filteredKategoriData[item.nama_entity_lemtera].realisasi += parseFloat(item.realisasi_nominal || 0);
                filteredKategoriData[item.nama_entity_lemtera].target += parseFloat(item.target_nominal || 0);
            ...
            function updateKategoriChart(kategoriData) {
                ...
                kategoriChart = new Chart(document.getElementById('kategoriChart'), {
                    ...
                    options: {
                        ...
                        maintainAspectRatio: false,
                        plugins: {
                            title: { display: true, text: 'Distribusi Kategori Lemtera' },
                            legend: { position: 'bottom' },
                            tooltip: {
                                callbacks: {
                                    label: function(context) {
                        ...
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title"><i class="fas fa-trash mr-2"></i>Hapus Data ${capitalizedType} lemtera</h5>
                                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                        <span aria-hidden="true">&times;</span>
                                    </button>
                                </div>
                                <div class="modal-body text-left">
                                    Anda yakin ingin menghapus data ${capitalizedType} lemtera <strong>${item.nama_program}</strong> Tanggal <strong>${new Date(item.tgl_surat).toLocaleDateString('id-ID')}</strong>?
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-default btn-round" data-dismiss="modal">Batal</button>
                                    <a href="modules/lemtera/proses_hapus.php?id=${id}" class="btn btn-danger btn-round delete-confirm">Ya, Hapus</a>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="modal fade" id="${modalId}" tabindex="-1" role="dialog" aria-labelledby="${modalId}Label" aria-hidden="true">
                    <div class="modal-dialog modal-lg" role="document">
                        <div class="modal-content">
                            ...
                            <form action="modules/lemtera/proses_ubah.php" method="POST" id="form${capitalizedType}${id}">
                                <div class="modal-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                <label class="form-label font-weight-semibold">
                                                    <i class="fas fa-tags mr-1 text-success"></i>Kategori lemtera <span class="text-danger">*</span>
                                                </label>
                                                <select class="form-control" name="entity_lemtera" required>
                                                    <option value="">-- Pilih Kategori --</option>
                                                    ${/* PERUBAHAN 1: Membuat dropdown Kategori dengan JavaScript */
                                                    allKategori.map(kategori => `
                                                        <option value="${kategori.id_entity}" ${kategori.id_entity == item.entity_lemtera ? 'selected' : ''}>
                                                            ${kategori.nama_entity_lemtera}
                                                        </option>
                                                    `).join('')}
                                                </select>
                                            </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <select class="form-control" name="status_lemtera" required>
                                                    <option value="">-- Pilih Status --</option>
                                                    ${/* PERUBAHAN 2: Membuat dropdown Status dengan JavaScript */
                                                    allStatus.map(status => `
                                                        <option value="${status.id_status}" ${status.id_status == item.status_lemtera ? 'selected' : ''}>
                                                            ${status.nama_status}
                                                        </option>
                                                    `).join('')}
                                                </select>
                                    ...
                                    <div class="alert alert-light border mt-3">
                                        ...
                                        <div class="row small">
                                            <div class="col-md-4 text-center">
                                                ...
                                                <span class="badge badge-primary">${item.nama_entity_lemtera || ''}</span>
                                            </div>
                                            <div class="col-md-4 text-center">
